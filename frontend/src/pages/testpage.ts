...
async function startTestGame(): Promise<void> {
  const currentUser = await getCurrentUser();
  showPlayerSetupModalTest(currentUser);
}

async function showPlayerSetupModalTest(currentUser: any): Promise<void> {
...
async function verifyPlayer2Test(currentUser: any): Promise<void> {
  const player2Name = (document.getElementById('player2Name') as HTMLInputElement).value.trim();
  const statusDiv = document.getElementById('player2Status');
  ...
  try {
    const playersResponse = await api<any>(`/api/database/players?search=${encodeURIComponent(player2Name)}&limit=10`);

    if (playersResponse.success && playersResponse.users) {
      const player = playersResponse.users.find((u: any) =>
        u.username === player2Name || u.display_name === player2Name
      );
...
function showLoginModalTest(player: any, currentUser: any): void {
  const modal = document.getElementById('modal')!;
  modal.classList.remove('hidden');
...
async function loginPlayer2Test(player: any, currentUser: any): Promise<void> {
  const email = (document.getElementById('player2Email') as HTMLInputElement).value.trim();
  const password = (document.getElementById('player2Password') as HTMLInputElement).value;
  ...
  try {
    const loginResponse = await api<any>('/api/auth/login', {
      method: 'POST',
...
function showPlayer2Verified(player: any, currentUser: any): void {
  const modal = document.getElementById('modal')!;
  modal.classList.remove('hidden');
  ...
  (window as any).startVerifiedGame = () => startVerifiedGameTest(player, currentUser);
}

async function startVerifiedGameTest(player2: any, currentUser: any): Promise<void> {
  hideModal();
...
function backToNamesModal(): void {
...
(window as any).backToNamesModal = backToNamesModal;
