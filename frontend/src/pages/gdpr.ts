...
export async function loadUserData(): Promise<any> {
  try {
    const result = await api<any>('/api/gdpr/user-data');
    return result.data;
  } catch (error: any) {
...
export async function exportUserData(): Promise<void> {
  try {
    ...
    showGDPRMessage(error.message || 'gdpr.exportError', 'error');
  }
}
...
export async function anonymizeUserData(): Promise<void> {
  try {
    ...
    showGDPRMessage(error.message || 'messages.connectionError', 'error');
  }
}
...
export async function loadUserConsent(): Promise<any> {
  try {
    const result = await api<any>('/api/gdpr/user-consent');
