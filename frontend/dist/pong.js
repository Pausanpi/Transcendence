...
let player1;
let player2;
const paddle1 = { x: 10, y: 250, w: 10, h: 100, dy: 0 };
const paddle2 = { x: 780, y: 250, w: 10, h: 100, dy: 0 };
 ...
 */
function predictBallYAtPaddle(ballState, paddleX) {
...
function update() {
    ...
    if (keys['s'] && paddle1.y < canvas.height - paddle1.h)
        paddle1.dy = PADDLE_SPEED;
    paddle1.y += paddle1.dy;
    // Clamp paddle1 position
    if (paddle1.y < 0)
        paddle1.y = 0;
    if (paddle1.y > canvas.height - paddle1.h)
    ...
    if (keys['ArrowDown'] && paddle2.y < canvas.height - paddle2.h)
        paddle2.dy = PADDLE_SPEED;
    paddle2.y += paddle2.dy;
    // Clamp paddle2 position
    if (paddle2.y < 0)
        paddle2.y = 0;
    if (paddle2.y > canvas.height - paddle2.h)
        paddle2.y = canvas.height - paddle2.h;
...
export function showWinnerOverlay(winnerName, onContinue) {
    const el = document.getElementById('countdown');
    const txt = document.getElementById('countdownText');
    el.classList.remove('hidden');
