...
async function uploadAvatar() {
    ...
    if (!avatarFile) {
        ...
        showProfileMessage(msg !== null ? msg : 'Please select a file', 'error');
        return;
    }
    ...
    if (!validTypes.includes(avatarFile.type) && !hasValidExtension) {
        ...
        showProfileMessage(msg !== null ? msg : 'Only JPG/JPEG files are allowed', 'error');
        return;
    }
    // Validate file size (2MB)
    if (avatarFile.size > 2 * 1024 * 1024) {
        const msg = window.languageManager?.t('profile.fileSizeLimit');
        showProfileMessage(msg !== null ? msg : 'File size must be less than 2MB', 'error');
        return;
    }
    ...
    try {
        ...
        showProfileMessage(uploadingMsg !== null ? uploadingMsg : 'Uploading...', 'success');
        ...
        showProfileMessage(successMsg !== null ? successMsg : (data.message || 'Avatar uploaded successfully'), 'success');
        ...
        showProfileMessage(errorMsg !== null ? errorMsg : (error.message || 'Failed to upload avatar'), 'error');
    }
}
...
async function updateProfile() {
    ...
    if (email && email.trim() !== '') {
        ...
        if (!emailRegex.test(email)) {
            ...
            showProfileMessage(msg !== null ? msg : 'Invalid email format', 'error');
            return;
        }
        ...
        if (tempEmailDomains.some(temp => domain.includes(temp))) {
            ...
            showProfileMessage(msg !== null ? msg : 'Temporary email addresses are not allowed', 'error');
            return;
        }
    }
    try {
        ...
        showProfileMessage(successMsg !== null ? successMsg : 'Email updated successfully', 'success');
...
async function deleteAvatar() {
    ...
    try {
        ...
        if (!token) {
            ...
            showProfileMessage(msg !== null ? msg : 'Please login first', 'error');
            return;
        }
        ...
        showProfileMessage(msg !== null ? msg : (data.message || 'Avatar removed successfully'), 'success');
        ...
        showProfileMessage(msg !== null ? msg : (error.message || 'Failed to remove avatar'), 'error');
    }
}
...
async function toggleMatchHistory() {
    ...
    if (!matchHistorySection || !toggleBtn)
        return;
...
async function loadMatchHistory() {
    ...
    try {
        ...
        const response = await api(`/api/database/players/${userId}`);
        if (response.success && response.user) {
...
window.toggleMatchHistory = toggleMatchHistory;
