...
async function startGateway() {
	...
	fastify.post('/auth/login', async (request, reply) => {
		...
		try {
			...
			if ((!setCookieHeaders || setCookieHeaders.length === 0) && upstreamRes.headers.get) {
				const singleCookie = upstreamRes.headers.get('set-cookie');
				if (singleCookie) setCookieHeaders = [singleCookie];
			}
			...
			return reply.code(upstreamRes.status).send(respData);
		} catch (err) {
